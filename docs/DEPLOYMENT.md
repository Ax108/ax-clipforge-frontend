...
Download needs the Express API on `:5000` ([https://github.com/Ax108/ax-clipforge-backend](https://github.com/Ax108/ax-clipforge-backend)). `VITE_API_URL` defaults to `http://localhost:5000/api/v1`. Load/title still works from oEmbed if the API is down.
...
A hosted UI **cannot** reach `localhost` or local Docker on your PC. You would need a **public HTTPS** ClipForge API (hosted container or reverse proxy) and:
